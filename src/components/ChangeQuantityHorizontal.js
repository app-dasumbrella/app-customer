...
export default class ChangeQuantityHorizontal extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  increase = () => {
    if (this.state.quantity < Constants.LimitAddToCart) {
      this.props.onChangeQuantity(this.state.quantity + 1, 'increase');
    }
  };

  reduced = () => {
    if (this.state.quantity >= 1) {
      this.props.onChangeQuantity(this.state.quantity - 1, 'decrease');
    }
  };
...
ChangeQuantityHorizontal.defaultProps = {
  quantity: 1,
  onChangeQuantity: () => { },
};
